...
get_summary_information <- function() {
  ...
  df_facility_filtered <- df_facility %>%
    ...
    group_by(Country) %>%
    summarise(
      "total avalibility per 100k population" = 
        sum(Mental.hospitals..per.100.000.population.,
  ...
  df_summary <- df_countries_filtered %>%
    mutate(
      "Mental Health Facility Avaliable(per 100k population)" = 
        df_facility_filtered$"total avalibility per 100k population"
    ) %>%
